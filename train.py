...
parser.add_argument('--content_attention_weight', type=float, default=7000)
...
for i in range(start_iter+1, args.stage1_iter+args.stage2_iter):
    ...
    if i < args.stage1_iter:
        stylized_results, loss_c,loss_attention, loss_s, loss_gan_d, loss_gan_g, loss_id, _ = network(content_images, style_images)
    else:
        stylized_results, loss_c,loss_attention, loss_s, loss_gan_d, loss_gan_g, loss_AR1, loss_AR2 = network(content_images, style_images, aesthetic=True)
    ...
    loss_attention=args.content_attention_weight*loss_attention
    loss_s = args.style_weight * loss_s
    loss_gan_g = args.gan_weight * loss_gan_g

    if i < args.stage1_iter:
        loss_id = args.identity_weight * loss_id
        loss = loss_c +loss_attention+ loss_s + loss_gan_g + loss_id
    else:
        loss_AR1 = args.AR1_weight * loss_AR1
        loss_AR2 = args.AR2_weight * loss_AR2
        loss = loss_c + loss_attention + loss_s + loss_gan_g + loss_AR1 + loss_AR2
    ...
    if (i + 1) % 500 == 0:
        ...
        print('[%d/%d] loss_content:%.4f,loss_attention:%.4f, loss_style:%.4f, loss_gan_g:%.4f, loss_gan_d:%.4f' % (i+1, args.stage1_iter+args.stage2_iter, loss_c.item(), loss_attention.item(), loss_s.item(), loss_gan_g.item(), loss_gan_d.item()))
